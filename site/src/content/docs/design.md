...
`@graphql-ts/schema` is intentionally designed to be a small library that just adds type-safety to the APIs in GraphQL.js used to construct a GraphQL schema.

There a few patterns sometimes used in solutions for that `@graphql-ts/schema` avoids:
