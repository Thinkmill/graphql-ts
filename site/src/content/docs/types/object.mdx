...
Since GraphQL, like the name suggests, is about querying a graph, it's common to have circular types in your schema where one type references another type that references the first type. If we try to naively define a circular type, we'll run into errors:

```ts twoslash  ins="() =>"/(User).,/
// @errors: 7022 7023
import { gWithContext } from "@graphql-ts/schema";
const g = gWithContext();
type g<T> = gWithContext.infer<T>;
...
The first one to solve is `Block-scoped variable 'User' used before its declaration.`. This is because, like the error suggests, we can't reference `User` before it's been declared. We can fix this by making `fields` a function that returns the fields so that it can be called lazily after the `User` type has been declared:
...
Now we still have TypeScript errors though, TypeScript doesn't understand this circularity. We can fix this by using the `g` type and the `g.object` function to be explicit about the type of `User`:
